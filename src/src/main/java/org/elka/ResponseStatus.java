...
public enum ResponseStatus {
    ...
    // Server Error
    INTERNAL_SERVER_ERROR(500, "Server is on a trip, please try again later"),
    ...
    public static @NotNull ResponseStatus fromHttpStatus(HttpStatus httpStatus) throws IllegalArgumentException {
        for (ResponseStatus status : values()) {
            if (status.code.equals(httpStatus.value())) {
                return status;
            }
        }
        throw new IllegalArgumentException("No matching ResponseStatus for HttpStatus " + httpStatus);
    }
